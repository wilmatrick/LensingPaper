...
\NEW{The central $v_\text{rms}$ dip is most likely a combination of the three different aspects that we investigated separately: We expect a bit of tangential velocity anisotropy (Sections \ref{sec:results_JAM_SB_MfL} and \ref{sec:results_JAM_NFW}), a moderate contribution of a roundish DM halo (Sections \ref{sec:results_lensing_compare} and \ref{sec:results_JAM_NFW_results}) and different stellar populations inside the bulge due to the merger (Section \ref{sec:discussion_merger}) with different $\Upsilon_{*}$ but overall bottom-heavy IMF (Section \ref{sec:MLdiscussion}), which add to an overall rising $\Upsilon_\text{tot}(R')$ profile.}
