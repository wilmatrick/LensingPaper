...
\NEW{As we only fitted the halo models to the inner regions, it is not surprising that they do not fit at larger radii. A fit to all nine available $v_\text{rms}$ data points would force the DM halo to be less massive. But with decreasing DM contribution in the inner regions the model approaches the mass-follows-light model in the center, that we already ruled out in Section \ref{sec:results_JAM_SB_MfL}. Allowing for free $\Upsilon_{*}(R')$, $\beta_z(R')$ and/or more flexible $\rho_\text{halo}(r)$ profiles, which would be the reasonable next step, would over-fit the data again. Without further priors on the stellar $\Upsilon_{*}(R')$ and/or more data points, we can make only the following statement: A model with constant $\Upsilon_*$, moderately tangential $\beta_z$ and a spherical and massive DM component with high concentration can explain the central kinematics of J1331 very well. We have however ruled out that in this case the DM halo will follow a NFW halo profile at both small and large radii.}
